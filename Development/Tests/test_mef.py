...
for fn in os.listdir(MODELS_D):
	...
	if b'HPRM' in mef.haschunk:
		datalist.extend(mef.hprm_1)



arr = np.array(datalist)

print(arr['_0'].min(), arr['_0'].max(), np.unique(arr['_0']).__len__())
print(arr['_1'].min(), arr['_1'].max(), np.unique(arr['_1']).__len__())
print(arr['_2'].min(), arr['_2'].max(), np.unique(arr['_2']).__len__())
print(arr['_3'].min(), arr['_3'].max(), np.unique(arr['_3']).__len__())
